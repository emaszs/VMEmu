...
public class Processing {
	public static void processCommand(String cmd) {
		...
		if (cmd.matches("LR\\d\\d") && RM.sm == 0) {
			...
			RM.sm = 0;
		} else if (cmd.matches("A2\\d\\d") && RM.sm == 0) {
			RM.sm = 1;
			int adr = Integer.parseInt(cmd.substring(2, 4));
			String valToAdd = Memory.getFromVirtualAddress(adr);
			...
			RM.sm = 0;
		} else if (cmd.matches("S1\\d\\d") && RM.sm == 0) {
			RM.sm = 1;
			int adr = Integer.parseInt(cmd.substring(2, 4));
			...
			RM.sm = 0;
		} else if (cmd.matches("S2\\d\\d") && RM.sm == 0) {
			RM.sm = 1;
			int adr = Integer.parseInt(cmd.substring(2, 4));
			String valToSub = Memory.getFromVirtualAddress(adr);
			String valR2 = RM.r2.getString();
			...
			RM.sm = 0;
		} else if (cmd.matches("ML\\d\\d") && RM.sm == 0) {
			RM.sm = 1;
			int adr = Integer.parseInt(cmd.substring(2, 4));
			...
			RM.sm = 0;
		} else if (cmd.matches("DV\\d\\d") && RM.sm == 0) {
			RM.sm = 1;
			int adr = Integer.parseInt(cmd.substring(2, 4));
			String valMemory = Memory.getFromVirtualAddress(adr);
			String valR1 = RM.r1.getString();
			...
			RM.r2 = RM.r1;
			RM.ic++;
		} else if (cmd.matches("CM\\d\\d") && RM.sm == 0) {
			RM.sm = 1;
		...
		} else if (cmd.matches("OU\\d\\d") && RM.sm == 0) { // output to printer
			int counter = 0;
			RM.si[2] = '1';

			// if output is just starting
			if (RM.chstPrinter == 0 && RM.sm == 0) {
				RM.chstPrinter = 1;
				RM.sm = 1;
