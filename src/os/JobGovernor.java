...
public class JobGovernor extends Process {
	...
	public JobGovernor(int intID, String extID, int parentProcess,
			int priority, String startState, Resource startingResource) {
	...
	public void run() {
		...
		if ((phase == 5) && (pState.equals("ru"))) {
			// TODO jobGov should contain only one child VM process (?) yes,
			// cause every task has its own JobGovernor which has its own VM
		...
		if ((phase == 6) && (pState.equals("ru"))) {
			MessageAboutIdentifiedInterrupt msg = (MessageAboutIdentifiedInterrupt) PyOS
					.findResource(this.ownedResList,
							"MessageAboutIdentifiedInterrupt");
