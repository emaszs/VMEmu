...
public class Interrupt extends Process {
	...
	public void run() {
		...
		if ((phase == 1) && (pState.equals("ru"))) {
			...
			Resource messageAboutInterrupt = PyOS.findResource(
					this.ownedResList, "MessageAboutInterrupt");

			// Finding parent process of VM that created interrupt
			Process vm = PyOS
					.findProcessByIntId(messageAboutInterrupt.creatorProcess);
		...
		if ((phase == 2) && (pState.equals("ru"))) {
			MessageAboutIdentifiedInterrupt newResource = new MessageAboutIdentifiedInterrupt(
					PyOS.id2++, "MessageAboutIdentifiedInterrupt", this.intID,
					interruptType, jobGovernorResponsibleForInt);

			this.createdResList.add(newResource);
			this.ownedResList.add(newResource);
			PyOS.resourceAmounts[10]++;
			PyOS.resourceList.add(newResource);
			PyOS.freeResource(PyOS.waitingList10, 10, newResource);
		}

		// 2-5) identify interrupt, identify JobGovernor, free Message about
